...
**Status: deployable, not deployed.** Everything needed to stand this up on AWS is committed — application code, infrastructure-as-code, CI/CD, and a GitHub OIDC trust so no long-lived AWS keys are ever stored in this repo. See [`docs/deployment.md`](docs/deployment.md) to bring it up.
