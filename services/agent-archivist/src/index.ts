...
class Archivist extends Agent<ArchivePayload, { archived: number; bytes: number }> {
...
startHealthServer(Number(process.env.PORT ?? 8080), log, () => ({ healthy: true, detail: { agent: 'archivist' } }));
void agent.start();
