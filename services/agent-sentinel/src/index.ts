...
class Sentinel extends Agent<SentinelPayload, { segments: number; anomalies: number; streams: number }> {
  ...
  private async scoreSegment(
    ...
    if (recent.length >= 3) {
      const { fingerprintSimilarity } = await import('@apex/core');
...
startHealthServer(Number(process.env.PORT ?? 8080), log, () => ({ healthy: true, detail: { agent: 'sentinel' } }));
void agent.start();
